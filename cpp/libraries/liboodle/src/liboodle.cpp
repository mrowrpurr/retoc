...
using OodleLZ_Compress = int64_t (*)(Compressor compressor,
                                     const uint8_t *rawBuf, size_t rawLen,
                                     uint8_t *compBuf, CompressionLevel level,
                                     void *pOptions, void *dictionaryBase,
                                     void *lrm, uint8_t *scratchMem,
                                     size_t scratchSize);

using OodleLZ_Decompress = int64_t (*)(
    const uint8_t *compBuf, size_t compBufSize, uint8_t *rawBuf, size_t rawLen,
    uint32_t fuzzSafe, uint32_t checkCRC, uint32_t verbosity,
    uint64_t decBufBase, size_t decBufSize, uint64_t fpCallback,
    uint64_t callbackUserData, uint8_t *decoderMemory, size_t decoderMemorySize,
    uint32_t threadPhase);

using OodleLZ_GetCompressedBufferSizeNeeded = size_t (*)(Compressor compressor,
                                                         size_t rawSize);
...
struct OodleLibrary::Impl {
  ...
  Impl()
      : library(nullptr), compress(nullptr), decompress(nullptr),
...
Result<HMODULE> loadOodleDll() {
  ...
  if (!file.good()) {
    return Result<HMODULE>(Error::IoError,
                           "Oodle DLL not found at: " + dllPath);
  }
...
OodleLibrary::OodleLibrary() : _impl(std::make_unique<Impl>()) {
  ...
  _impl->getCompressedBufferSizeNeeded =
      reinterpret_cast<OodleLZ_GetCompressedBufferSizeNeeded>(GetProcAddress(
          _impl->library, "OodleLZ_GetCompressedBufferSizeNeeded"));
...
OodleLibrary::compress(const std::vector<uint8_t> &input, Compressor compressor,
...
Result<std::vector<uint8_t>>
OodleLibrary::compress(const uint8_t *input, size_t inputSize,
  ...
  // Compress the data
  int64_t compressedSize = _impl->compress(compressor, input, inputSize,
                                           buffer.data(), compressionLevel,
                                           nullptr, // pOptions
  ...
  if (compressedSize == -1) {
    return Result<std::vector<uint8_t>>(Error::CompressionFailed,
                                        "Oodle compression failed");
  }
...
Result<std::vector<uint8_t>>
OodleLibrary::decompress(const std::vector<uint8_t> &input, size_t outputSize) {

  std::vector<uint8_t> output(outputSize);

  auto result =
      decompress(input.data(), input.size(), output.data(), outputSize);
  if (result.isError()) {
...
Result<size_t> OodleLibrary::decompress(const uint8_t *input, size_t inputSize,
                                        uint8_t *output, size_t outputSize) {

  int64_t decompressedSize =
      _impl->decompress(input, inputSize, output, outputSize,
                        1,       // fuzzSafe
                        1,       // checkCRC
                        0,       // verbosity
                        0,       // decBufBase
  ...
  if (decompressedSize <= 0) {
    return Result<size_t>(Error::CompressionFailed,
...
size_t OodleLibrary::getCompressedBufferSizeNeeded(Compressor compressor,
                                                   size_t rawSize) {
