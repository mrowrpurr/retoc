...
int main() {
    ...
    if (oodleResult.isError()) {
        std::cerr << "Failed to initialize Oodle: " << oodleResult.errorMessage() << std::endl;
        return 1;
    }
    ...
    const std::string testData = 
        ...
        "OodleXLZ_Compress_AsyncAndWait (in the Oodle2 Ext lib) instead to get parallelism. "
        "Alternatively, chop the data into small fixed size chunks (we recommend at least 256KiB, "
        "i.e. 262144 bytes) and call compress on each of them, which decreases compression ratio "
        "but makes for trivial parallel compression and decompression.";
    
    std::vector<uint8_t> inputData(testData.begin(), testData.end());
    
    // Compress the data
    auto compressResult = oodle->compress(
        inputData, 
        oodle::Compressor::Mermaid, 
        oodle::CompressionLevel::Optimal5);
    
    if (compressResult.isError()) {
        std::cerr << "Compression failed: " << compressResult.errorMessage() << std::endl;
        return 1;
    }
    ...
    std::cout << "Compression ratio: " << static_cast<float>(inputData.size()) / compressedData.size() << std::endl;
    ...
    if (matches) {
        ...
        std::cerr << "Decompression failed: data does not match original" << std::endl;
        return 1;
    }
    
    // Test with raw pointers
    auto compressResult2 = oodle->compress(
        inputData.data(),
        inputData.size(),
    ...
    if (compressResult2.isError()) {
        std::cerr << "Compression (pointer version) failed: " << compressResult2.errorMessage() << std::endl;
        return 1;
    }
    ...
    std::cout << "Original size: " << inputData.size() << " bytes" << std::endl;
    std::cout << "Compressed size: " << compressedData2.size() << " bytes" << std::endl;
    std::cout << "Compression ratio: " << static_cast<float>(inputData.size()) / compressedData2.size() << std::endl;
    
    // Decompress using the raw pointer version
    std::vector<uint8_t> decompressedData2(inputData.size());
    auto decompressResult2 = oodle->decompress(
        compressedData2.data(),
        compressedData2.size(),
        decompressedData2.data(),
        decompressedData2.size());
    
    if (decompressResult2.isError()) {
        std::cerr << "Decompression (pointer version) failed: " << decompressResult2.errorMessage() << std::endl;
        return 1;
    }
    ...
    if (matches) {
        std::cout << "Decompression (pointer version) successful: data matches original" << std::endl;
    } else {
        std::cerr << "Decompression (pointer version) failed: data does not match original" << std::endl;
        return 1;
    }
